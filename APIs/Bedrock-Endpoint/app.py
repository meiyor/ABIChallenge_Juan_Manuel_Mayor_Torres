...
checkpoint_pruning = torch.load("./models/model_checkpoint_Trial-2024-10-06-063816461703-ixds2")
...
bedrock_runtime = boto3.client('bedrock-runtime', region_name='us-east-1', aws_access_key_id=os.environ.get("AWS_ACCESS_KEY"), aws_secret_access_key=os.environ.get("AWS_SECRET_KEY"))
...
def ini():
    ...
    data_cumm = []
    img_cumm = []
    message =  {"answer": "Let's start having an interaction with the chatbot.. <br>"}
...
def predict(item=Item, db=db):
   ...
   if interactions % 5 == 0:
      ...
      register = item(id=id, code=code_str, date=time_after, username=username, Interaction_Register=data_cumm, Images_Files=img_cumm)
      data_cumm = []
      img_cumm = []
